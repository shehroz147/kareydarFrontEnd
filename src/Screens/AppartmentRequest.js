...
export default function AppartmentRequest({ route }) {
    ...
    const getPendingRequest = async () => {
        ...
        const value1 = {
            appartmentId: property._id
        }
        const value2 = {
            roomId: property._id
        }

        const response = await server.post('RequestAppartment/UserRequest/', value);
    ...
    const acceptRequest = async (requestId) => {
        ...
        if (response.status === 200) {
            Linking.openURL(
                `http://api.whatsapp.com/send?phone=${requests.user.phone}`
